...
<script>
  ...
  import alarmReestr from "~/store/modules/alarmReestr";
  ...
  export default {
    ...
    mounted: function () {
      let alarmReestr = funcUtils.getfromLocalStorage('alarmReestr');
      let getData = (methodName) => {
        let cid = alarmReestr;
        let requestHead = new RequestEntity.RequestHead(localStorage.getItem('sid'), wid, cid, this.$store.state.alarmReestr.bean, methodName);
      ...
      if (funcUtils.isNull(alarmReestr)) {
        let requestHead = new RequestEntity.RequestHead(localStorage.getItem('sid'), wid, null, this.$store.state.alarmReestr.bean, null);
              ...
              if (data.length > 0) {
                ...
                if (!funcUtils.isNull(respData)) {
                  if (dataJson.method === 'addCID') {
                    this.$store.dispatch('alarmReestrSetCid', respData.cid);
                    alarmReestr = respData.cid;
                    funcUtils.addToLocalStorage('alarmReestr', alarmReestr);
                    getData('getData');
                  }
                } else {
    ...
    methods: {
      updateOnCommand: function (resp) {
        let wid = sessionStorage.getItem('wid');
        let requestHead = new RequestEntity.RequestHead(localStorage.getItem('sid'), wid, resp.cid, this.$store.state.alarmReestr.bean, 'restore');
